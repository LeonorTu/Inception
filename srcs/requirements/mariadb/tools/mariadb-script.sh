...
# Select the system database containing user tables
USE mysql;
# Apply privilege changes immediately
FLUSH PRIVILEGES;

# Set root password using environment variable from .env
ALTER USER 'root'@'localhost' IDENTIFIED BY "$MYSQL_ROOT_PASSWORD";
# Create WordPress database with UTF-8 support for international characters
CREATE DATABASE $WORDPRESS_DATABASE_NAME CHARACTER SET utf8 COLLATE utf8_general_ci;
# Create WordPress user with access from any host (% wildcard)
# The ability to connect from any host is necessary for container networking
CREATE USER $WORDPRESS_DATABASE_USER@'%' IDENTIFIED BY "$WORDPRESS_DATABASE_PASSWORD";
# Grant WordPress user full access to the WordPress database
